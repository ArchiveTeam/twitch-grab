...
LUA_SHA1 = get_hash(os.path.join(CWD, 'twitch.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type in ('video', 'novideo'):
                wget_args.extend(['--warc-header', 'twitch-video: '+item_value])
                wget_args.append('https://www.twitch.tv/videos/'+item_value)
                if len(item['device_id']) == 0:
                    response = requests.get(wget_args[-1])
                    item['device_id'] = response.cookies['unique_id']
                if item_type == 'novideo':
                    item['novideos'].append(item_value)
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'twitch-asset: '+url])
                wget_args.append(url)
            else:
                raise Exception('Unknown item')

        item['novideos'] = ','.join(item['novideos'])
...
project = Project(
    ...
    project_html='''
        ...
        <h2>Twitch <span class="links"><a href="https://twitch.tv/">Website</a> &middot; <a href="https://tracker.archiveteam.org/twitch/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Twitch">Wiki</a></span></h2>
...
pipeline = Pipeline(
    ...
    WgetDownload(
        ...
        env={
            ...
            'novideos': ItemValue('novideos'),
            'device_id': ItemValue('device_id')
        }
    ),
    SetBadUrls(),
